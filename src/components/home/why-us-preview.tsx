  ...
  return (
    <section className="py-8 sm:py-16 md:py-24 bg-white border-b border-slate-100">
      <Container className="space-y-6 sm:space-y-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 sm:gap-8 lg:gap-10 lg:items-stretch items-center">
          {/* Left Column: Benefit Grid + CTA Button */}
          <div className="lg:col-span-7 flex flex-col justify-between space-y-4 sm:space-y-8">
            <div className="grid grid-cols-2 sm:grid-cols-2 gap-2.5 sm:gap-4">
              {items.slice(0, 4).map((item) => (
                <div
                  key={item.id}
                  className="p-3 sm:p-5 rounded-xl sm:rounded-2xl bg-slate-50 border border-slate-100 space-y-1.5 sm:space-y-2 hover:border-emerald-200 transition-colors"
                >
                  <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-lg sm:rounded-xl bg-light-green flex items-center justify-center">
                    {getIcon(item.icon_name)}
                  </div>
                  <h3 className="text-[11px] sm:text-sm font-bold text-navy">
                    {item.title}
                  </h3>
            ...
            {/* CTA hidden on mobile — Trial CTA and Final CTA provide conversion */}
          ...
          <div className="lg:col-span-5 w-full">
            {whyUsImage?.secure_url ? (
              <>
                {/* Desktop: bordered card with overlay */}
                <div className="hidden sm:block relative rounded-3xl overflow-hidden shadow-2xl border border-slate-200 group lg:h-full">
                  <img
                ...
                {/* Mobile: integrated image with fade, no borders */}
                <div className="block sm:hidden w-full overflow-hidden select-none pointer-events-none">
                  <img
                    src={whyUsImage.secure_url}
                    alt={whyUsImage.alt_text || "Why Choose Yolfin Group"}
                    className="w-full h-auto max-h-[200px] object-cover object-center hero-mask-image"
                  />
                </div>
              </>
            ) : (
              /* Corporate Fallback Card when no custom image assigned */
              <div className="relative rounded-2xl sm:rounded-3xl bg-gradient-to-br from-navy via-slate-900 to-navy-dark p-5 sm:p-8 text-white shadow-xl overflow-hidden border border-slate-800 space-y-4 sm:space-y-6 lg:h-full flex flex-col justify-between h-auto">
                <div className="space-y-4 sm:space-y-6">
                  <div className="w-12 h-12 sm:w-16 sm:h-16 rounded-xl sm:rounded-2xl bg-brand-green/20 border border-emerald-500/30 flex items-center justify-center">
                    <Handshake className="w-6 h-6 sm:w-8 sm:h-8 text-emerald-400" />
                  </div>

                  <div className="space-y-2 sm:space-y-3">
                    <span className="inline-block px-3 py-1 bg-emerald-950 text-emerald-300 border border-emerald-800 text-[10px] sm:text-[11px] font-bold rounded-full uppercase tracking-wider">
                      Trusted Business Partner
                    </span>
                ...
                <div className="pt-3 sm:pt-4 border-t border-slate-800 flex flex-wrap items-center justify-between gap-2 text-[10px] sm:text-xs text-slate-400 font-medium">
