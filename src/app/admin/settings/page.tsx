...
import { Save, Loader2, Mail, Phone, MapPin, Building2, MousePointerClick, Share2 } from "lucide-react";
...
export default function AdminSettingsPage() {
    ...
    const fetchSettings = async () => {
      try {
        ...
        if (res.ok && mounted) {
          ...
          if (map.social_instagram_url) setInstagramUrl(typeof map.social_instagram_url === "string" ? map.social_instagram_url : String(map.social_instagram_url));
          if (map.social_facebook_url) setFacebookUrl(typeof map.social_facebook_url === "string" ? map.social_facebook_url : String(map.social_facebook_url));
          if (map.social_twitter_url) setTwitterUrl(typeof map.social_twitter_url === "string" ? map.social_twitter_url : String(map.social_twitter_url));
        }
      } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const [res1, res2, res3, res4, res5, res6, res7, res8] = await Promise.all([
      ...
      updateSiteSettingAction("social_instagram_url", instagramUrl),
      updateSiteSettingAction("social_facebook_url", facebookUrl),
      updateSiteSettingAction("social_twitter_url", twitterUrl),
    ]);

    setIsSubmitting(false);

    if (res1.error || res2.error || res3.error || res4.error || res5.error || res6.error || res7.error || res8.error) {
    ...
    <div className="space-y-8 pb-12 max-w-4xl">
      ...
      <div className="border-b border-slate-800 pb-6">
        ...
        <p className="text-slate-400 text-xs sm:text-sm">
          Update global CTA button text, public corporate contact info, social media URLs, phone numbers, email and headquarters address.
        </p>
      </div>
      ...
      {isLoading ? (
        ...
        <form onSubmit={handleSubmit} className="bg-slate-900 border border-slate-800 rounded-3xl p-6 sm:p-8 space-y-8 shadow-xl">
          ...
          <div className="space-y-4 pt-2">
            ...
            <p className="text-xs text-slate-400">
              Configure public social media profile URLs for the website footer. Leave empty to hide unconfigured social icons.
            </p>

            <div className="space-y-4">
              <div>
                <label className="block text-xs font-bold uppercase text-slate-400 mb-1.5 flex items-center gap-1.5">
                  <InstagramIcon className="w-3.5 h-3.5 text-emerald-400" /> Instagram URL
                </label>
                <input
                  type="url"
                  value={instagramUrl}
                  onChange={(e) => setInstagramUrl(e.target.value)}
                  placeholder="https://instagram.com/yolfingroup"
                  className="w-full px-4 py-3 bg-slate-950 border border-slate-800 rounded-xl text-sm text-slate-100 focus:outline-none focus:ring-2 focus:ring-emerald-500"
                />
              </div>
              ...
              <div>
                <label className="block text-xs font-bold uppercase text-slate-400 mb-1.5 flex items-center gap-1.5">
                  <FacebookIcon className="w-3.5 h-3.5 text-emerald-400" /> Facebook URL
                </label>
                <input
                  type="url"
                  value={facebookUrl}
                  onChange={(e) => setFacebookUrl(e.target.value)}
                  placeholder="https://facebook.com/yolfingroup"
                  className="w-full px-4 py-3 bg-slate-950 border border-slate-800 rounded-xl text-sm text-slate-100 focus:outline-none focus:ring-2 focus:ring-emerald-500"
                />
              </div>
              ...
              <div>
                <label className="block text-xs font-bold uppercase text-slate-400 mb-1.5 flex items-center gap-1.5">
                  <Share2 className="w-3.5 h-3.5 text-emerald-400" /> Twitter/X URL
                </label>
                <input
                  type="url"
                  value={twitterUrl}
                  onChange={(e) => setTwitterUrl(e.target.value)}
                  placeholder="https://x.com/yolfingroup"
                  className="w-full px-4 py-3 bg-slate-950 border border-slate-800 rounded-xl text-sm text-slate-100 focus:outline-none focus:ring-2 focus:ring-emerald-500"
                />
              </div>
            </div>
